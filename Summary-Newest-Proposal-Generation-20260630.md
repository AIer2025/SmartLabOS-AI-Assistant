...
### B8. WORD 重生成脚本 `build_docx.js` 的使用（含 `npm run docx`）

> 「方案生成」阶段二会让 Claude 现编一个 Node 脚本 `build_docx.js` 并执行，产出最终 .docx。
> 本节说明该脚本今后如何复用，以及为免去手敲环境变量而补充的 npm 脚本。

**脚本性质**
- 用 `docx` npm 库**逐段组装** .docx 的 Node 脚本；输出路径硬编码在文末（`build_docx.js:775-776`）。
- 正文内容**写死在脚本里**（文字 / 34 张表 / 章节均内联）。开头注释的"数据源 `1.html / 2.html / 模块选定-推荐.json`"只是**出处标注**——运行时并不读取这些文件（全脚本无 `readFileSync`/`JSON.parse`）。
- 因此它是**本项目本次成果的快照**，不会自动跟随 `references/` 或 HTML 变化。

**本机运行环境（已确认）**
- Node `v24.15.0`；`docx` `9.7.1` 装在**全局** `C:\Users\littl\AppData\Roaming\npm\node_modules`。
- 直接 `node build_docx.js` 会报 `Cannot find module 'docx'`（脚本在项目目录、docx 在全局，Node 默认不搜全局），必须指定 `NODE_PATH`。

**新增便捷脚本（本次补充）**
- 在 `projects/测试项目/` 新增 `package.json`，内置 `NODE_PATH`，今后重生成只需：

  ```powershell
  cd "C:\TestClaude\SmartLabOS-AI-Assistant\projects\测试项目"
  npm run docx
  ```

  其中脚本定义为 `set "NODE_PATH=C:\Users\littl\AppData\Roaming\npm\node_modules" && node build_docx.js`（npm 在 Windows 经 cmd 执行，`set` 即时生效，无需手动设环境变量）。
- **已实测**：`npm run docx` 正常输出 `WROTE …_20260630160022.docx (41194 bytes)`；重生成文件经 `zipfile` 校验 `is_zipfile=True`、`testzip=None(OK)`、**26 个部件**、XML 全部 well-formed、含 `word/document.xml`，与首版等价（仅 zip 元数据相差 1 字节，属正常）。

**今后的典型用法**
1. **小幅修订**：编辑脚本对应章节的 JS（文字/指标/表格），`npm run docx` 覆盖重生成。
2. **改输出名/目录**：改 `build_docx.js:775` 的 `outPath`（否则会覆盖现有 docx）。
3. **替换占位图**：当前"图 X-X"为文字占位，需插实图时在脚本里用 `ImageRun` 引入 CAD 底图（清单见文档附录 A）。
