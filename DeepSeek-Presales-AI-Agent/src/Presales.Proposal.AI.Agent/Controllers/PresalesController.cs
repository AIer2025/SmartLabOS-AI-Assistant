...
/// 但内部由「Messages API 直连 + 后台队列/工作者」实现，取代「CLI 子进程」。
/// 打到哪家大模型由 Agent:Provider / Agent:BaseUrl 决定，本层与供应商无关。
/// </summary>
[ApiController]
[Route("api/presales")]
public sealed class PresalesController : ControllerBase
{
    ...
    public PresalesController(PresalesRepository repo, PresalesPaths paths, ModuleCatalog catalog,
        LlmAgentService agent, GenerationQueue queue, JobRegistry jobs, AgentEnvFile envFile,
    ...
    public async Task<IActionResult> SelectModules(long id)
    {
        ...
        if (!_agent.HasApiKey) return BadRequest(new { message = _agent.MissingApiKeyMessage });
    ...
    public async Task<IActionResult> Generate(long id)
    {
        ...
        EnsureDir(p);
        // 记进供应商/模型：切换前后的运行记录混在一张表里时，这是事后对比质量的唯一区分依据。
        var commandFile = $"{_agent.ProviderLabel}-{DateTime.Now:yyyyMMdd-HHmmss}";
