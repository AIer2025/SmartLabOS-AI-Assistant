...
(() => {
  ...
  function genPhaseText(phase) {
    if (phase >= 2) return "阶段 2/2：整合详细设计方案并转出 WORD(.docx)";
    if (phase === 1) return "阶段 1/2：生成解决方案 HTML 与 URS";
    return "准备中：组织指令并调用大模型…";
  }
  // s = 生成状态响应；据其更新进度块
  ...
  async function inferModules() {
    ...
    if (!confirm("将调用大模型，依据国标与需求自动推理推荐模块。\n该过程可能需要一段时间，期间可继续查看日志。是否开始？")) return;
    try {
  ...
  async function generate() {
    ...
    if (!confirm("将调用大模型在项目目录下自动生成提案文件：\n阶段一：解决方案 HTML 与（必要时）模块 URS；\n阶段二：《详细设计方案》Markdown → 自动转出带封面/目录/页码的 WORD(.docx)。\n两阶段过程可能需要较长时间。是否开始？")) return;
    try {
  ...
  // 交付物分类：与会话方式的产物形态一一对应，缺项直接高亮，不必翻日志
  const FILE_GROUPS = [
    { key: "solution", label: "解决方案 HTML", icon: "📄", test: (n) => /^\d+\.html?$/i.test(n) },
    { key: "urs",      label: "模块 URS",      icon: "📋", test: (n) => /URS/i.test(n) },
    { key: "docx",     label: "WORD 提案",     icon: "📝", test: (n) => /\.docx$/i.test(n) },
    { key: "detail",   label: "详细设计 MD",   icon: "📑", test: (n) => /详细设计方案\.md$/i.test(n) },
    { key: "summary",  label: "输出总结",      icon: "🧾", test: (n) => /^Summary.*\.md$/i.test(n) },
    { key: "runlog",   label: "运行日志",      icon: "🧰", test: (n) => /\.log$/i.test(n) },
    { key: "other",    label: "其它",          icon: "📎", test: () => true },
  ];
  ...
  function fileChip(name) {
    ...
    if (isDocx) a.download = name;   // WORD 下载而非内联打开
  ...
  function renderFiles(files) {
    ...
    const buckets = new Map(FILE_GROUPS.map((g) => [g.key, []]));
    names.forEach((n) => {
      const g = FILE_GROUPS.find((g) => g.test(n));
      buckets.get(g.key).push(n);
    });

    // 期望的解决方案 HTML 份数 = 已选流程标准数
    const expectedHtml = collectChecks("psProtocols").length;

    FILE_GROUPS.forEach((g) => {
      const items = buckets.get(g.key);
      const missing = g.key === "solution" && expectedHtml > items.length ? expectedHtml - items.length : 0;
      if (!items.length && !missing) return;

      const row = document.createElement("div");
      row.className = "ps-file-group";

      const label = document.createElement("span");
      label.className = "ps-file-label";
      label.textContent = `${g.icon} ${g.label}`;
      row.appendChild(label);

      items.forEach((n) => row.appendChild(fileChip(n)));
      if (missing) {
        const warn = document.createElement("span");
        warn.className = "ps-file ps-file-missing";
        warn.textContent = `缺 ${missing} 份（共需 ${expectedHtml} 份）`;
