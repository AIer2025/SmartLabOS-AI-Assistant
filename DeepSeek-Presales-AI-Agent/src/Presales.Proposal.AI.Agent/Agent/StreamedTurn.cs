...
internal sealed class StreamedTurn
{
    ...
    /// </summary>
    /// <param name="dropRedactedThinking">
    /// 是否丢弃 redacted_thinking 内容块而不回传。DeepSeek 的 Anthropic 兼容端点**不支持**该内容块，
    /// 一旦模型产出它、我们又照原样回传，下一轮整个请求会 400。概率低，但一次就够让整个任务判失败，
    /// 所以在这里直接掐掉：丢一个不可读的思考块，远好过丢掉整轮生成。
    /// </param>
    public static async Task<StreamedTurn> RunAsync(AnthropicClient client, MessageCreateParams parameters,
        int maxAttempts, Action<string> log, CancellationToken ct, bool dropRedactedThinking = false)
    {
        ...
        for (int attempt = 1; ; attempt++)
        {
            try
            {
                return await StreamOnceAsync(client, parameters, ct, dropRedactedThinking);
            }
    ...
    ///
    /// DeepSeek 供应商下请求里根本不出现 cache_control（见 LlmAgentService.Cache()），
    /// 此时集合为空、本检查自然通过，无需额外分支。
    ///
    ...
    internal static void ValidateCacheControl(MessageCreateParams parameters)
    {
        ...
        if (ttls.Count > 1)
            throw new InvalidOperationException(
                ...
                + "请检查是否有 cache_control 绕过了 LlmAgentService.Cache() 这个唯一出口。");
    }

    /// <summary>
    /// 可重试判定：服务端过载/限流/网络抖动可重试；鉴权与请求本身的错误重试多少次都一样。
    ///
    /// **异常类型名那半边与供应商无关**：SDK 按 HTTP 状态码映射异常，打 DeepSeek 时抛的仍是
    /// AnthropicRateLimitException / Anthropic5xxException 这些类型，因此判定继续有效。
    /// 失效的只是错误**文案**匹配 —— DeepSeek 返回自己的错误体，不会出现 overloaded_error 这类
    /// Anthropic 专有 type 串，故下面把两家的关键字都列上。
    /// </summary>
    private static bool IsRetryable(Exception ex)
    {
        ...
        if (msg.Contains("authentication_error") || msg.Contains("invalid_request_error")
            ...
            // DeepSeek：鉴权失败 / 余额不足 / 参数错误 —— 重试多少次都一样，早点报出来
            || msg.Contains("Authentication Fails") || msg.Contains("Insufficient Balance")
        ...
        return msg.Contains("overloaded_error") || msg.Contains("rate_limit_error")
            ...
            // DeepSeek：限流与服务端繁忙
            || msg.Contains("Rate Limit Reached") || msg.Contains("Server Overloaded")
    ...
    private static string Brief(Exception ex)
    {
        ...
        if (msg.Contains("overloaded_error") || msg.Contains("Server Overloaded")) return "服务端过载";
        if (msg.Contains("rate_limit_error") || msg.Contains("Rate Limit Reached")) return "触发限流";
        if (msg.Contains("Insufficient Balance")) return "账户余额不足（DeepSeek）";
        if (msg.Contains("Authentication Fails")) return "密钥无效（DeepSeek）";
    ...
    private static async Task<StreamedTurn> StreamOnceAsync(AnthropicClient client, MessageCreateParams parameters,
        CancellationToken ct, bool dropRedactedThinking = false)
    {
        ...
        {
            var param = b.Build(out var toolCall, dropRedactedThinking);
    ...
    private sealed class BlockBuilder
    {
        ...
        /// <param name="dropRedactedThinking">true 时丢弃 redacted_thinking（DeepSeek 兼容端点不支持该内容块）。</param>
        public ContentBlockParam? Build(out ToolCall? toolCall, bool dropRedactedThinking = false)
        {
            toolCall = null;
            switch (Kind)
            {
                ...
                case "redacted_thinking":
                    // DeepSeek 兼容端点不支持该内容块：回传即 400。丢弃它只损失一段本就不可读的思考，
                    // 保住的是整轮生成——这个取舍在任何时候都划算。
                    if (dropRedactedThinking) return null;
