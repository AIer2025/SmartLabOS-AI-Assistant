...
/// <summary>
/// SmartLabOS 售前提案 AI Agent 的大模型核心 —— 用 Anthropic Messages API 的请求形状直接调用大模型。
/// 要点：
///   - 「模块选定」用**强制工具调用**拿到机器可读的结构化结果（两家都支持，见 <see cref="SelectModulesAsync"/>）；
///   - 「方案生成」用**手写 Agent 工具循环**(受控 read/write 工具)，模型自主读知识库、写提案文件；
///   - 稳定的知识库上下文放在 **system 前缀**，两家都能靠前缀缓存降本（Anthropic 显式 cache_control，
///     DeepSeek 磁盘自动缓存），故前缀必须逐字节稳定；
///   - 强类型异常、可拿到 usage（token 用量）便于成本观测。
///
/// **供应商无关**：实际打到哪家由 <see cref="AgentSettings.BaseUrl"/> 决定 ——
/// 留空走 Anthropic 官方，填 <c>https://api.deepseek.com/anthropic</c> 走 DeepSeek 的 Anthropic 兼容端点。
/// DeepSeek 兼容层与官方 API 有三处差异，本类已按 <see cref="AgentSettings.IsDeepSeek"/> 分流：
///   ① 忽略 <c>output_config.format</c>（结构化输出）→ 模块选定改用强制工具调用；
///   ② 忽略 <c>cache_control</c> → 不再发送该字段，避免无谓的未知字段；
///   ③ 不支持 <c>redacted_thinking</c> 内容块 → 由 <see cref="StreamedTurn"/> 在回传时剔除。
/// </summary>
public sealed class LlmAgentService
{
    ...
    private readonly ILogger<LlmAgentService> _log;

    private string? _templateCache;
    private string? _detailTemplateCache;

    /// <summary>模块选定所用工具名。定义、tool_choice、结果读取三处共用，避免写歪。</summary>
    private const string SubmitModulesTool = "submit_modules";
    ...
    ///
    /// **DeepSeek 下返回 null**：兼容端点明确忽略 cache_control，改由其磁盘自动缓存按前缀完全匹配接管。
    /// 返回 null 即整个请求不出现该字段 —— 少一个未知字段就少一分被拒的风险，
    /// 且本系统"稳定 system 前缀在前、对话历史在后"的结构本就是自动前缀缓存的理想形态。
    /// </summary>
    private CacheControlEphemeral? Cache()
    {
        if (_s.IsDeepSeek) return null;
        return string.Equals(_s.CacheTtl, "5m", StringComparison.OrdinalIgnoreCase)
            ? new CacheControlEphemeral()                        // 默认 5 分钟
            : new CacheControlEphemeral { Ttl = Ttl.Ttl1h };
    }

    /// <summary>
    /// 思考配置。adaptive 是 Anthropic 的自适应思考配置形状。
    ///
    /// 2026-08-15 实测：兼容端点接受 <c>{"type":"adaptive"}</c>（200）；但 V4 Pro 本就**恒开推理**
    /// —— 不发该字段时每次响应同样带 thinking 块 —— 所以发与不发行为一致。既然无收益，
    /// 就默认 off，少发一个未知字段少一分风险。需要时改配置 Agent:Thinking 即可，无需重新编译。
    ///
    /// 恒开推理还有一个连带后果，见 SelectModulesAsync 里 ToolChoiceAny 的注释：
    /// thinking 模式下不接受"指定某个具体工具"的 tool_choice。
    /// </summary>
    private ThinkingConfigParam? ThinkingParam()
    {
        if (!string.Equals(_s.Thinking, "adaptive", StringComparison.OrdinalIgnoreCase)) return null;
        return new ThinkingConfigAdaptive();
    }

    public LlmAgentService(AgentSettings settings, PresalesPaths paths, ModuleCatalog catalog,
        ProposalTemplateSet templates, DocxExporter docx, IConfiguration config, ILogger<LlmAgentService> log)
    {
    ...
    public bool HasApiKey => !string.IsNullOrWhiteSpace(AgentSettings.ResolveApiKey(_config, _s.Provider));

    /// <summary>Key 的脱敏描述（来源 + 长度 + 掩码 + 供应商匹配性），供启动自检日志排查「读到的不是我设的那把」。</summary>
    public string ApiKeyDescription =>
        AgentSettings.Describe(AgentSettings.ResolveApiKeyInfo(_config, _s.Provider), _s.Provider);

    /// <summary>供应商+模型的简短标识，用于运行记录命名与前端展示（如 deepseek/deepseek-v4-pro）。</summary>
    public string ProviderLabel => $"{_s.Provider}/{_s.Model}";

    /// <summary>缺 Key 时给前端/接口的统一提示（按供应商给出该去哪儿配）。</summary>
    public string MissingApiKeyMessage => _s.IsDeepSeek
        ? "未配置 DeepSeek API Key，无法调用大模型。请在 env/Presales-AI-Agent-config.json 的 DeepSeek_Env:DeepSeek_API_Key 填入密钥，或设置环境变量 DEEPSEEK_API_KEY。"
        : "未配置 Anthropic API Key，无法调用大模型。请设置环境变量 ANTHROPIC_API_KEY。";

    private AnthropicClient CreateClient()
    {
        var key = AgentSettings.ResolveApiKey(_config, _s.Provider);
        if (string.IsNullOrWhiteSpace(key))
            throw new InvalidOperationException(MissingApiKeyMessage);

        // 唯一的接入点切换处：留空走 Anthropic 官方，填 DeepSeek 兼容端点走 DeepSeek。
        // 改配置即可整体回退，不需要重新编译，也不需要动下面任何一行业务代码。
        // BaseUrl 是 init-only 属性，只能在对象初始化器里给，故分两支构造而不是先建后赋。
        return string.IsNullOrWhiteSpace(_s.BaseUrl)
            ? new AnthropicClient { ApiKey = key }
            : new AnthropicClient { ApiKey = key, BaseUrl = _s.BaseUrl };
    }
    ...
    ///
    /// **取结构化结果的方式按供应商分流**：
    ///   - DeepSeek：兼容端点忽略 <c>output_config.format</c>，改用 <c>tools + tool_choice</c> 强制调用
    ///     <c>submit_modules</c>，参数即为原来的 JSON Schema —— 约束力与结构化输出相当；
    ///   - Anthropic：沿用 <c>OutputConfig.Format</c>，保持改造前行为不变，便于随时切回对照。
    /// 两条路径最终都汇到同一套解析与知识库校验，兜底逻辑（正则抽取）也共用。
    /// </summary>
    public async Task<ModulePick> SelectModulesAsync(PresalesProject p, Action<string> log, CancellationToken ct)
    {
        var client = CreateClient();
        var useToolCall = _s.IsDeepSeek;
        log(useToolCall
            ? $"==== 模块选定：强制工具调用（tool_choice=any → {SubmitModulesTool}）· {_s.Provider}/{_s.Model} ===="
            : $"==== 模块选定：结构化输出（OutputConfig.Format）· {_s.Provider}/{_s.Model} ====");

        var user = BuildModuleSelectionPrompt(p, log);
        var effort = ParseEffort(_s.Effort);
        var system = BuildKnowledgeSystem();

        // MessageCreateParams 的 OutputConfig / Tools / ToolChoice 都是 init-only，
        // 只能在对象初始化器里给，所以两条路径各构造一次，而不是先建对象再按分支赋值。
        var parameters = useToolCall
            ? new MessageCreateParams
            {
                ...
                Tools = [BuildSubmitModulesTool()],
                // 用 any 而不是 tool：**实测** DeepSeek V4 Pro 恒开 thinking，而 thinking 模式下
                // 指定具体工具会被拒 —— HTTP 400 "Thinking mode does not support this tool_choice"。
                // any 表示"必须调用某个工具"，而本次请求只定义了 submit_modules 这一个工具，
                // 因此语义与指定它完全等价，且实测可用。
                // 【勿改回 ToolChoiceTool】除非将来这一步会同时提供多个工具。
            ...
            : new MessageCreateParams
            {
                ...
                {
                    Effort = effort,
                    Format = new JsonOutputFormat { Schema = BuildModuleSchema() },
                },
            };
        ...
        try
        {
            turn = await StreamedTurn.RunAsync(client, parameters, _s.MaxAttempts, log, ct, _s.IsDeepSeek);
        }
        catch (Exception ex)
        {
        ...
        var raw = ReadModulePayload(turn, useToolCall, log);
        log($"[原文] 内容块={turn.BlockSummary} 工具调用={turn.ToolCalls.Count} 载荷长度={raw.Length}");
        log($"[原文] {Flatten(Truncate(raw, 1200))}");

        var pick = SafeParse<ModulePick>(raw);
        ...
        if (ids.Count == 0)
        {
            // 工具调用被忽略时（理论上不该发生），文本块里也可能带着结果，一并纳入正则抽取范围。
            ids = _catalog.ExtractFromText(raw.Length > 0 ? raw + "\n" + turn.Text : turn.Text);
            if (ids.Count > 0)
    ...
    /// <summary>
    /// 模块选定的提交工具。参数 Schema 与结构化输出用的是同一套字段定义，
    /// 只是换了一种让模型必须遵守的方式 —— DeepSeek 兼容端点忽略 output_config.format，
    /// 但完整支持 tool_choice 的 tool 变体，约束力相当。
    /// </summary>
    private static Tool BuildSubmitModulesTool() => new()
    {
        Name = SubmitModulesTool,
        Description = "提交最终选定的模块清单。必须调用本工具给出结果，不要用普通文本回答。",
        InputSchema = new()
        {
            Properties = Props(
                ("modules", JsonSerializer.SerializeToElement(new
                {
                    type = "array",
                    items = new { type = "string" },
                    description = "推荐模块ID数组，按前处理流程顺序排列，形如 MOD-CC-001",
                })),
                ("notes", Str("选型理由与未覆盖工序说明(可选)"))),
            Required = ["modules"],
        },
    };

    /// <summary>模块选定的 JSON Schema —— 结构化输出路径（anthropic 供应商）使用。</summary>
    private static Dictionary<string, JsonElement> BuildModuleSchema() => new()
    {
        ["type"] = J("object"),
    ...
    /// <summary>
    /// 从本回合取出模块选定的 JSON 载荷：工具调用路径读 tool_use 的 input，结构化输出路径读文本块。
    /// 工具路径下若模型没调工具（被忽略/被拒），退回读文本，交给上层的正则兜底处理。
    /// </summary>
    private static string ReadModulePayload(StreamedTurn turn, bool useToolCall, Action<string> log)
    {
        if (!useToolCall) return turn.Text;

        var call = turn.ToolCalls.FirstOrDefault(c =>
            string.Equals(c.Name, SubmitModulesTool, StringComparison.OrdinalIgnoreCase));

        if (call.Name is null || call.Input is null)
        {
            log("[警告] 模型未按要求调用 submit_modules 工具，改从返回文本中解析。");
            return turn.Text;
        }

        // 把 tool_use 的 input 字典重新序列化成 JSON，复用下游同一套反序列化逻辑。
        return JsonSerializer.Serialize(call.Input);
    }
        ...
        for (int iter = 1; iter <= _s.MaxToolIterations; iter++)
        {
            ...
            try
            {
                turn = await StreamedTurn.RunAsync(client, new MessageCreateParams
                {
                    ...
                    // DeepSeek 下 Cache() 返回 null，该字段不出现在请求里，由其磁盘自动缓存接管。
                    CacheControl = Cache(),
                }, _s.MaxAttempts, log, ct, _s.IsDeepSeek);
            }
    ...
    /// <summary>稳定的知识库 system 前缀（Anthropic 下带缓存标记）：目录导航 + 全量模块清单 + 输出模版 + 硬约束。</summary>
    private List<TextBlockParam> BuildKnowledgeSystem()
    {
        ...
        // 「严禁编造」是否定式约束，对模型只说了不许做什么、没给出该做什么。补一条正面授权：
        // 读不到就照实写"待补"，并明确这是被鼓励的正确做法。给出合法退出路径，比只堵死更有效。
        sb.AppendLine("   读不到某个参数时，请直接写「该参数需向知识库补充：<参数名>」并继续后续内容——");
        sb.AppendLine("   这是被鼓励的正确做法，不是失败；凭印象填一个看似合理的数值才是严重错误。");
        ...
        // 注意：下面是近百条格式高度相似的条目，最容易诱发"顺手编一个看着合理的编号"。
        // 因此在清单前后各钉一句硬约束，别指望模型自己记得上面那条。
        sb.AppendLine("【重要】只能使用本清单中逐字出现过的模块ID，一个字符都不能改。清单里没有的编号一律视为不存在。");
        foreach (var m in _catalog.All())
            sb.AppendLine($"- {m.Id}  {m.Name}{(string.IsNullOrWhiteSpace(m.Category) ? "" : $"（{m.Category}）")}");
        sb.AppendLine("【重要】以上清单之外的任何模块ID都是编造的，会被程序直接过滤掉，请勿输出。");
        sb.AppendLine();
    ...
    private string BuildModuleSelectionPrompt(PresalesProject p, Action<string> log)
    {
        ...
        sb.AppendLine();
        // 本步骤是**单次调用，不提供读写工具**（系统提示里的 read_kb_file / list_kb_dir 只在后续
        // 「方案生成」阶段可用）。因此国标全文必须在此内联给出——否则模型只拿到文件名，无从判断前处理
        // 流程，只能输出占位符敷衍（实测返回 {"modules":["NEED_TO_READ_PROTOCOL"]}）。
        sb.AppendLine("【重要】本步骤你无法读取任何文件。所需国标全文已在下方内联给出，");
        ...
        sb.AppendLine();
        sb.AppendLine(_s.IsDeepSeek
            ? $"请调用 {SubmitModulesTool} 工具提交结果（modules 为按流程顺序排列的模块ID数组，notes 可选），不要用普通文本回答。"
            : "请只输出符合给定 JSON Schema 的结果（modules 为按流程顺序排列的模块ID数组，notes 可选）。");
        return sb.ToString();
    }

    /// <summary>供前端「预览」用：返回阶段一将发给模型的任务提示词（不含知识库系统前缀）。</summary>
    ...
    /// 可直接传原始字符串 "xhigh"（Anthropic Opus 4.7+ 支持）。
    ///
    /// **DeepSeek 注意**：xhigh 是 Anthropic 自 Opus 4.7 起的专有档位。2026-08-15 实测，
    /// 兼容端点对 high 与 xhigh 都返回 200 不报错，但是否真正影响推理深度无法从外部观测，
    /// 故 deepseek 供应商下配置默认仍给 "high"（保守），需要时改配置 Agent:Effort 即可。
    /// </summary>
    private static ApiEnum<string, Effort> ParseEffort(string s) => (s ?? "high").Trim().ToLowerInvariant() switch
    {
        "low" => Effort.Low,
    ...
    private void LogUsage(StreamedTurn turn, Action<string> log)
    {
        var totalIn = turn.InputTokens + turn.CacheReadTokens + turn.CacheWriteTokens;

        if (_s.IsDeepSeek)
        {
            // DeepSeek 兼容端点不回填 Anthropic 的缓存计量字段，这两个数恒为 0 —— 显式说明，
            // 免得运维看到"缓存读=0"误判为缓存失效而去乱调参数（其自动缓存是否命中在此不可见）。
            log($"[用量] 输入={totalIn} 输出={turn.OutputTokens} stop={turn.StopReason}"
              + "  （DeepSeek 自动磁盘缓存，命中情况 API 不回传，此处不可见）");
            return;
        }
