...
// MySQL 连接参数、知识库运行目录、DeepSeek API Key 全部来自该文件；以内存配置层追加，
// 优先级高于 appsettings.json 与环境变量——现场只认这一份文件，杜绝
// 「改了 JSON 却被残留的 ConnectionStrings__SmartLabOS 环境变量顶掉」这类幽灵问题。
// 密钥同理：这台机器上很可能还留着旧的 ANTHROPIC_API_KEY，让文件优先才不会被它顶掉。
...
try
{
    ...
    var agent = app.Services.GetRequiredService<LlmAgentService>();
    var settings = app.Services.GetRequiredService<AgentSettings>();
    ...
    // 大模型接入参数逐项打出来：换供应商后最常见的故障是"配是配了、但生效的不是那一套"，
    // 把供应商/接入点/模型/档位摆在启动日志里，比事后翻请求体快得多。
    app.Logger.LogInformation("大模型：{Summary}", settings.Summary());
    app.Logger.LogInformation("大模型 API Key：{Key}", agent.ApiKeyDescription);
    if (!agent.HasApiKey)
        app.Logger.LogError("未读到大模型 API Key —— 模块选定与方案生成将无法执行。{Hint}", agent.MissingApiKeyMessage);
}
