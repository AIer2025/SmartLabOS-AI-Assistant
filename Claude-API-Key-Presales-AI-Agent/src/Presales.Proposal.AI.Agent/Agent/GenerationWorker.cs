...
public sealed class GenerationWorker : BackgroundService
{
    ...
    public GenerationWorker(GenerationQueue queue, JobRegistry registry, PresalesRepository repo,
        ClaudeAgentService agent, AgentSettings settings, ILogger<GenerationWorker> log)
    {
    ...
    /// <summary>写进日志文件头部的运行参数，便于事后核对这次用的是什么模型/档位。</summary>
    private string RunHeader() =>
        $"模型={_settings.Model}  努力档位={_settings.Effort}  MaxTokens={_settings.MaxTokens}  "
      + $"缓存TTL={_settings.CacheTtl}  最大尝试={_settings.MaxAttempts}  "
